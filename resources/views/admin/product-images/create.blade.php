        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <div class="flex justify-between items-center">
                    <h2 class="text-xl font-semibold text-gray-800">Add New Image for {{ $product->name }}</h2>
                ...
                <form action="{{ route('admin.products.images.store', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative border-2 border-dashed border-gray-300 rounded-lg p-6 transition-all duration-200 ease-in-out bg-gray-50 hover:bg-gray-100" id="dropzone">
                            <!-- Hidden file input -->
                            <input id="image" name="image" type="file" class="absolute inset-0 w-full h-full opacity-0 z-50 cursor-pointer" required accept="image/*">
                            ...
                            <div id="preview-container" class="mt-2 hidden" style="display: none; flex-direction: column; align-items: center;">
                                <img id="preview-image" class="max-h-64 max-w-full rounded border border-gray-200 object-contain shadow-sm" alt="Image preview" src="">
                        ...
                        <p class="text-sm text-gray-500">Recommended size: 800x800 pixels, Max: 2MB</p>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="alt_text" class="block text-sm font-medium text-gray-700">Alt Text</label>
                            <input type="text" name="alt_text" id="alt_text" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Description of the image for SEO and accessibility" value="{{ old('alt_text') }}">
                            <p class="mt-1 text-sm text-gray-500">Describe the image for search engines and screen readers</p>
                        </div>
                        
                        <div>
                            <label for="sort_order" class="block text-sm font-medium text-gray-700">Sort Order</label>
                            <input type="number" name="sort_order" id="sort_order" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="{{ old('sort_order', 0) }}" min="0">
                    ...
                    <div class="relative flex items-start">
                        <div class="flex items-center h-5">
                            <input id="is_primary" name="is_primary" type="checkbox" value="1" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded" {{ old('is_primary') ? 'checked' : '' }}>
                        </div>

...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="bg-white shadow-sm rounded-lg p-6">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">Update the blog post using the form below.</p>
        </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea name="excerpt" id="excerpt" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $blogPost->excerpt) }}</textarea>
                ...
                <div class="mb-6 relative">
                    ...
                    <div class="mt-1 flex items-center">
                        <div class="relative h-64 w-full border-2 border-gray-300 border-dashed rounded-lg flex justify-center items-center overflow-hidden bg-gray-50">
                            ...
                            <img src="{{ $blogPost->featured_image_url }}" alt="{{ $blogPost->title }}" class="h-full w-full object-cover" id="image-preview">
                            @else
                            ...
                            <div class="absolute bottom-2 right-2 bg-gray-900 bg-opacity-75 text-white py-1 px-3 rounded text-sm">
                                <label for="featured_image">
                                    {{ $blogPost->featured_image ? 'Change Image' : 'Choose Image' }}
                                </label>
                            </div>
                        </div>
                    </div>
                    @if($blogPost->featured_image)
                    <p class="mt-1 text-xs text-gray-500">Current image: {{ basename($blogPost->featured_image) }}</p>
                    @endif
                    ...
                    <div class="ml-3 text-sm">
                        ...
                        <p class="text-gray-500">
                            @if($blogPost->published)
                                Currently published{{ $blogPost->published_at ? ' on ' . $blogPost->published_at->format('M d, Y') : '' }}
                            @else
